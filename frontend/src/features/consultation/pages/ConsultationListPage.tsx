...
import {
  PenLine, Eye, BarChart3, ClipboardList, Trash2,
...
// ── Note row (tab Semua Catatan — table layout sama dengan Isi Catatan) ────────

function NoteRow({ note, isViewOnly = false }: { note: ConsultationNote; isViewOnly?: boolean }) {
  const [confirmOpen, setConfirmOpen] = useState(false);
  const deleteMutation = useDeleteConsultationNote();

  const services = (note.invoice?.items ?? []).map((i) => i.item.name).join(", ");
      ...
      <tr className="hover:bg-muted/20 transition-colors group">
        <td className="px-4 py-3">
          ...
          {note.customer?.mobilePhone && (
            <div className="text-xs text-muted-foreground">{note.customer.mobilePhone}</div>
          )}
        </td>
        <td className="px-4 py-3 hidden sm:table-cell">
          <div className="text-sm text-muted-foreground truncate max-w-[160px]">{services || "—"}</div>
          {note.invoice?.invoiceNo && (
            <div className="text-xs font-mono text-muted-foreground/70">{note.invoice.invoiceNo}</div>
          )}
        </td>
        <td className="px-4 py-3 hidden md:table-cell">
          {note.interestingNote ? (
            <p className="text-xs text-muted-foreground line-clamp-2 italic max-w-[200px]">
              "{note.interestingNote}"
            </p>
          ) : <span className="text-xs text-muted-foreground">—</span>}
        </td>
        <td className="px-4 py-3 whitespace-nowrap hidden sm:table-cell">
          <div className="text-sm text-muted-foreground">{formatDate(note.filledAt)}</div>
        </td>
        <td className="px-4 py-3 text-center">
          {note.isNewClient !== undefined && <ClientTypeBadge isNewClient={note.isNewClient} />}
        </td>
              ...
              <div className="rounded-xl border border-border bg-card overflow-hidden overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-muted/40">
                      <th className="px-4 py-2.5 text-left font-semibold text-foreground/70 text-xs uppercase tracking-wide">Pelanggan</th>
                      <th className="px-4 py-2.5 text-left font-semibold text-foreground/70 text-xs uppercase tracking-wide hidden sm:table-cell">Layanan</th>
                      <th className="px-4 py-2.5 text-left font-semibold text-foreground/70 text-xs uppercase tracking-wide hidden md:table-cell">Catatan</th>
                      <th className="px-4 py-2.5 text-left font-semibold text-foreground/70 text-xs uppercase tracking-wide hidden sm:table-cell">Tanggal</th>
                      <th className="px-4 py-2.5 text-center font-semibold text-foreground/70 text-xs uppercase tracking-wide">Tipe</th>
                      <th className="px-4 py-2.5 text-right font-semibold text-foreground/70 text-xs uppercase tracking-wide">Aksi</th>
                    </tr>
                  ...
                  <tbody className="divide-y divide-border/60">
                    {notes.map((note) => <NoteRow key={note.id} note={note} isViewOnly={isViewOnly} />)}
                  </tbody>
                </table>
              </div>
            )}
